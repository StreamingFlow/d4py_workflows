...
mpiexec -n 6 --allow-run-as-root --oversubscribe  dispel4py mpi download_FDSN.py -f download_test.json -n 6

# ####### 4. Get pre-processed synth and data --- Misfit Preprocess

mpiexec -n 17 --allow-run-as-root --oversubscribe dispel4py mpi create_misfit_prep.py -n 17

# ####### 5. Get ground motion parameters and compare them

searchpath="./misfit_data/output/"
mpiexec -n 7 --allow-run-as-root --oversubscribe  dispel4py mpi dispel4py_RA.pgm_story.py -d '{"streamProducerReal": [ {"input":"'$searchpath'" } ], "streamProducerSynth": [ {"input": "'$searchpath'"} ]}' -n 7
